...
class ScoreSimulateRequest(BaseModel):
    ...
    hiring_trend: Optional[str] = None           # "Growing" | "Stable" | "Declining"
    website_engagement: Optional[str] = None     # "High" | "Medium" | "Low"
    email_open_rate: Optional[str] = None        # "Opened" | "Sent but Not Opened" | "No Outreach"
    email_reply: Optional[str] = None            # "Replied" | "No Reply"
    linkedin_engagement: Optional[str] = None     # "Engaged" | "No Engagement"
    recent_news: Optional[str] = None            # "Positive News" | "No News"
...
def calculate_lead_score(
    ...
    """
    Computes a lead score from 0-100 based on rules and weights.
    Returns (final_score, explanation_breakdown).
    """
    overrides = overrides or {}

    # 1. Company Size (Weight: 15)
    ...
    if hiring_override is not None:
        ...
        # Infer hiring trend from insight text
        has_hiring = False
        if insight:
    ...
    if web_override is not None:
        ...
        # Check interactions for website visit
        visits = [i for i in interactions if i.interaction_type.lower() in ["website", "visit", "website visit"]]
        if len(visits) >= 5:
            score_website = 10
        elif len(visits) >= 1:
            score_website = 6
    ...
    if email_open_override is not None:
        ...
        # Check if email is opened in interactions
        has_sent = any(c.campaign_status == "sent" for c in campaigns)
        has_opened = any(i.interaction_type.lower() in ["email opened", "open", "email open"] for i in interactions)
        if has_opened:
            score_email_open = 10
        elif has_sent:
            score_email_open = 5
    ...
    # Demo Booked Booster (Booster: +15 points, capped at 100)
    demo_val = overrides.get("demo_booked")
    if demo_val is None:
        demo_val = any("demo" in i.interaction_type.lower() or "meeting" in i.interaction_type.lower() for i in interactions)
...
def generate_ai_recommendation(
    ...
    """Invokes the LLM to generate highly contextual, actionable suggestions."""
    if _client is None:
...
def generate_lead_score_endpoint(lead_id: int, db: Session = Depends(get_db)):
    """Calculates, saves and returns the official score and recommendations for a lead."""
    lead = db.query(Lead).filter(Lead.lead_id == lead_id).first()
    if not lead:
        raise HTTPException(status_code=404, detail="Lead not found.")

    # Gather data from earlier modules
...
def get_lead_score_endpoint(lead_id: int, db: Session = Depends(get_db)):
    """Retrieves the stored lead score. If not generated, calculates it on the fly."""
    score_rec = db.query(LeadScore).filter(LeadScore.lead_id == lead_id).first()
    if not score_rec:
...
def simulate_lead_score_endpoint(req: ScoreSimulateRequest, db: Session = Depends(get_db)):
    """Runs a simulated What-If score and recommendation generation without writing to database."""
...
def get_ranked_leads_endpoint(db: Session = Depends(get_db)):
    """Retrieves all leads along with their scores, sorted by score descending."""
    ...
    for l in leads:
        ...
        if not score_rec:
            # Generate one on the fly to build ranking dynamically
            try:
                # Run calc logic without saving to keep it fast, or save it
